;(function () {
  ...
  $(document.body)
    ...
    .on('blur', '[name="sourceURL"]', function (e) {
      var tlTag = $(this).parents('.hvl-pedit-form:first')
        .find('.hvl-pedit-tags .hvl-pedit-tags-from .hvl-tag-tl')

      var msg = $(this).attr('data-prompt')

      if (tlTag.length && $.trim($(this).val()) != '' && msg) {
        $(this).attr('data-prompt', '')
        confirm(msg) && tlTag.click()
      }
    })
    ...
    .on('click', '.hvl-poll-option [name="votes[]"]', function (e) {
      ...
      if (e.target.value < 0) {
        // "Abstain from voting".
        var toUncheck = $(e.target).parents('.hvl-poll-option:first').prevAll('.hvl-poll-option')
      } else {
        var toUncheck = $(e.target).parents('.hvl-poll:first').find('.hvl-poll-option:last')
      }

      toUncheck.find('[type="checkbox"]').each(function () { this.checked = false })
    })
